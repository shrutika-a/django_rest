...
from login.models import AuthUser,EmployeePrimdetail,Daksmsstatus
...
from login_rest.permissions import CsrfExemptSessionAuthentication
...
class OtpConfirmationView(APIView):
	def post(self,request):
		...
		'''matching=Daksmsstatus.objects.filter(phonenos=phone_number).order_by('-mainid')[:1]
		if matching:
			if matching[0].otp == otp:
				checker_date = timezone.now() - timedelta(minutes=15)
				if matching[0].rectimestamp >= checker_date:	
